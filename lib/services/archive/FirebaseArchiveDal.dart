...
class FirebaseArchiveDal implements IFirebaseArchiveDal {
  ...
  Future<void> add(Note note) async {
    try {
      final DocumentReference<Map<String, dynamic>> ref =
          _firestore.collection('archive').doc(note.id);
  ...
  Future<void> delete(Note note) async {
    try {
      await _firestore.collection('archive').doc(note.id).delete();
    } catch (e) {
      developer.log('\x1B[31m$e\x1B[0m');
    }
  }
